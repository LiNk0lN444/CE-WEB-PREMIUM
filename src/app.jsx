...
export default function App() {
  ...
  const handleLoginSuccess = (usuario) => {
    ...
    alert(`¡Bienvenido de nuevo, ${usuario.username || 'Usuario'}!`);
  ...
  return (
    <div className={`app-root ${darkMode ? 'theme-dark' : 'theme-light'}`}>
      <header className="navbar-global">
        ...
        <nav>
          <ul className="nav-menu-links">
            <li>
              <button className={vistaActual === 'inicio' ? 'active' : ''} onClick={() => setVistaActual('inicio')}>
                Inicio
              </button>
            </li>
            <li>
            ...
            <li>
              {usuarioSesion ? (
                <button className="btn-hero-secondary" style={{ padding: '0.4rem 0.8rem', fontSize: '0.85rem' }} onClick={handleCerrarSesion}>
                  👤 {usuarioSesion.username || 'Usuario'} (Salir)
                </button>
              ) : (
                <button className={vistaActual === 'login' ? 'active' : ''} onClick={() => setVistaActual('login')}>
                  🔑 Iniciar Sesión
                </button>
              )}
            </li>
      ...
      <main className="main-content-container">
                ...
                <div className="hero-text-side">
                  <div className="welcome-badge">
                    ...
                    <span className="welcome-text">
                      {usuarioSesion ? `¡Hola de nuevo, ${usuarioSesion.username || 'Usuario'}!` : '¡Hola, bienvenido a CE-Web!'}
                    </span>
        ...
        {vistaActual === 'cotizaciones' && <Cotizaciones darkMode={darkMode} onNavigateLogin={() => setVistaActual('login')} />}
        {vistaActual === 'inventario' && <Inventario darkMode={darkMode} />}
      </main>
    </div>
  );
}